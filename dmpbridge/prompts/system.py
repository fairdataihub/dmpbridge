...
You will use the JSON array of blocks copied below and generated by processing the PDF of a DMP using a text extraction tool that provides a block ID, related text, and associated structural information (bold, italic, location, etc.). Note that blocks could be generated per line of text extracted from the PDF and not necessarily per paragraph, so you should consider successive blocks when annotating to label with the full context. Some blocks could also have multiple labels (e.g., question.text for the question and answer.text for the answer if they are on the same line). 
Return a JSON array with exactly ONE entry per labeled text in the following format: {id, text_labelled, label, confidence}, where id is the id of the block and text_labelled is the text from that block that is labeled. A confidence score must be a numerical value (between 0 and 1) that reflects your confidence in the label assigned to each block. Keep the response limited to the requested JSON structure, with no extra explanations, comments, or additional text. 
