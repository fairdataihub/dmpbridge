...
You will receive a JSON array of blocks generated by processing the PDF of a DMP using a text extraction tool that provides the text and associated structural information (bold, italic, location, etc.). Note that blocks could be generated per line of the PDF and not necessarily per paragraph, so you should consider connecting blocks when annotating.

Return a JSON array with exactly ONE entry per block: {id, label, confidence}. A confidence score must be a numerical value (between 0 and 1) that reflects your confidence in the label assigned to each block.
"""
