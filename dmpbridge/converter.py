...
_INLINE_Q      = re.compile(r'^[A-Z][^.\n]{2,40}\.\s+[A-Z][a-z]')  # "Label. Description sentence..."
...
def preprocess_blocks(blocks: list[dict]) -> list[dict]:
    ...
    # Rule 5: long section.title that is not numbered and comes after sections already exist → answer.text
    #   e.g. "Data Volume. Generally, the volume of data generated by CBES researchers..."
    seen_section = False
    for b in blocks:
        if b.get('label') == 'section.title':
            text = b.get('text', '').strip()
            if seen_section and len(text) > 40 and not _DIGIT_PREFIX.match(text) and not _INLINE_Q.match(text):
                b['label'] = 'answer.text'
            seen_section = True
