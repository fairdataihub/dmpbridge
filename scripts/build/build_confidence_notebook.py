...
cells = [
    ...
    md("md-5-sd", [
        "### The same two columns, with their spread",
        "",
        "An average alone can hide a lot. **SD** (standard deviation) says how much the",
        "individual items vary around it — a small SD means the items are clustered near the",
        "average, a large one means they are spread out.",
    ]),
    code("result-sd", [
        "def mean_sd(values):",
        "    \"\"\"'89.4 (7.2)' — the average, with the spread in brackets.\"\"\"",
        ...
        "         mean_sd(d[d['correct']]['confidence']),",
        "     'Confidence when incorrect (%), mean (SD)':",
        "         mean_sd(d[~d['correct']]['confidence'])}",
        "    for model, d in ((m, df[df['model'] == m]) for m in MODELS)",
        "]).set_index('Model')",
        "display(spread)",
    ]),
    md("md-5-sd-b", [
        "The two columns overlap heavily: for every model the gap between the averages is",
        "smaller than the spread within each group. In other words, plenty of **wrong** items",
        "are more confident than plenty of **right** ones — so no threshold separates them.",
    ]),
    md("md-5b", [
