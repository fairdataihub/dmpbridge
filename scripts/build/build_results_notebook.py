...
cells = [
    ...
    md("md-3e", [
        ...
        "",
        "The matrix above is a summary — every count in it collapses down to some number of",
        "individual items. This opens that back up: **every item that is not a clean diagonal",
        "match**, wrong-label or missing entirely, listed with which sample it's from and the",
        "actual text involved. Built from the exact same `_match_structured()` call the matrix",
        "itself is built from, so a row here and a cell above can never disagree.",
    ]),
    code("matrix-detail", [
        "pd.set_option('display.max_colwidth', None)",
        "",
        "",
        "def matrix_detail_rows(path):",
        "    \"\"\"One row per item that is NOT a clean diagonal match in the confusion",
        "    matrix: either labeled as something other than its true class, or",
        "    produced nothing at all ('(not labeled)'). Diagonal (correct) items",
        ...
        "        records, _ = _match_structured(pred_path, gold, dedup_question_title=dedup,",
        "                                       threshold=THRESHOLD)",
        "        for r in records:",
        ...
        "    detail = matrix_detail_rows(path)",
        "    print(f'{name}: {len(detail)} item(s) off the diagonal (matrix total minus the diagonal)')",
        "    if len(detail):",
        "        display(detail.sort_values(['true label', 'sample'])",
