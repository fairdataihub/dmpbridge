...
cells = [
    ...
    # ── 3b — every off-diagonal / not-labeled / extra item, traced to real text ─
    md("md-3e", [
        "### 3b. Every non-diagonal, `(not labeled)`, and extra item, traced to its actual text",
        "",
        ...
        "a clean diagonal match**, listed with which sample it's from and the actual text",
        "involved. Three kinds of row appear:",
        "",
        "| true label | predicted label | meaning |",
        "|---|---|---|",
        "| a real class | a different real class | wrong label — matched, but mislabeled |",
        "| a real class | `(not labeled)` | nothing matched this real item at all |",
        "| `(none)` | a real class | a predicted item with no real item to match — invisible",
        "| | | to the matrix above, since the matrix has no row for something that isn't real |",
        "",
        "Built from the exact same `_match_structured()` call the matrix itself is built from —",
        "including its second return value, the unclaimed predictions the matrix's own row/column",
        "structure has no way to show — so this table and the matrix can never disagree, and",
        "nothing that happened to a document is left invisible between the two.",
    ]),
    code("matrix-detail", [
        ...
        "    \"\"\"One row per Path B item that is NOT a clean diagonal match: labeled as",
        "    something other than its true class, produced nothing at all",
        "    ('(not labeled)'), or a predicted item with no real item to match",
        "    ('(none)' true label) — the last of these has no cell in the matrix above",
        "    at all, since the matrix is indexed by true label and this has none.",
        "    Diagonal (correct) items are not listed here.\"\"\"",
        "    rows = []",
        "    for n in range(1, 11):",
        ...
        "            rows.append({'sample': n, 'true label': '(none)',",
        "                         'predicted label': pred_label, 'text': pred_text})",
        "    return pd.DataFrame(rows, columns=['sample', 'true label', 'predicted label', 'text'])",
        "",
        "",
        "detail = matrix_detail_rows()",
        "print(f'Path B: {len(detail)} item(s) not a clean diagonal match',",
        "      f'({(detail[\"true label\"] != \"(none)\").sum()} traceable to a matrix cell,',",
        "      f'{(detail[\"true label\"] == \"(none)\").sum()} extra items invisible to the matrix)')",
        "if len(detail):",
    ...
    md("md-3f", [
        ...
        "checked against something concrete rather than taken on faith. And for the `(none)` rows",
        "specifically, the matrix cannot tell you about them **at all** — they are the model",
        "producing something the answer key never asked for, so section 1's precision figure is",
        "the only place their effect shows up otherwise.",
    ]),
]
