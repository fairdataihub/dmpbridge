...
TAGS = {
    ...
    "Docling text blob + gemma4:e4b":    "gemma4-e4b_docling_whole_doc",
...
cells = [
    ...
    md("md-3b", [
        ...
        "  This is the same finding as `investigation-docling-native-json-sample2.ipynb`:",
        "  Docling reads the fonts into its page cells and never carries them upward.",
    ...
    md("md-4", [
        ...
        "underline. The hierarchy has none of the three; the blob has all of them. That is",
        "why the ranking is what it is:",
        "",
        "| representation | classifier | Path A |",
        "|---|---|---|",
        "| Docling hierarchy | rules | 0.709 |",
        "| Docling text (headings only) as a blob | gemma4:e4b | 0.767 |",
        "| pdfplumber blob with all three markers | gemma4:e4b | **0.946** |",
        "",
        "Adding a model to Docling's text buys only six points over the rules, because the",
        "model has nothing more to read than the rules did — and the gain is not even",
        "uniform: gemma recovers samples 1, 2 and 3 where the rules fail, but *loses* on",
        "samples 5 and 8, which the rules label perfectly (1.000 vs 0.538 and 0.615).",
        "Adding the markers buys eighteen points, and pdfplumber + gemma is at or above",
        "the rules on every document.",
        "",
        "**Recommendation.** Keep the annotated blob as the representation the model",
        "classifies. Docling's hierarchy is not a replacement for it, and reading Docling's",
        "JSON instead of its Markdown would change nothing (section 1 of the companion",
        "notebook). Two things from the native output are worth having *alongside* the",
        "blob if the pipeline ever needs them, neither of which affects the score:",
        "page and bounding-box provenance for every block (the blob has no idea where",
        "text came from), and the furniture layer for dropping repeated page headers.",
        "A hybrid that keeps pdfplumber's markers and adds Docling's `section_header`",
        "labels as a second signal is the one untested combination; the per-document",
        "table suggests it would help on samples 4–9, which are already perfect, and not",
        "on 1, 2, 6, where the losses are typographic.",
    ]),
]
