...
cells = [
    ...
    code("matrix-counts", [
        ...
        "                             squeeze=False)",
        "    for c, m in enumerate(available):",
        "        ax, cm = axes[0][c], build_counts(CONF[m][path])",
        "        row_max = cm.max(axis=1, keepdims=True)",
        "        # shaded by share of ITS OWN row max, purely to guide the eye —",
        "        # the printed number is always the real count, never a percentage",
        "        shade = np.divide(cm, row_max, out=np.zeros_like(cm, dtype=float),",
        "                          where=row_max > 0)",
        "        im = ax.imshow(shade, cmap='Blues', vmin=0, vmax=1)",
        "        for i in range(len(LABELS)):",
        "            for j in range(len(KEYS)):",
        "                v = int(cm[i, j])",
        ...
        "                        color='#b9b9b4' if v == 0",
        "                        else ('white' if shade[i, j] > 0.55 else INK))",
        "        ax.set_xticks(range(len(COLS)))",
        "        ax.set_yticks(range(len(LABELS)))",
        "        ax.set_xticklabels(COLS, rotation=30, ha='right')",
        "        ax.set_yticklabels(LABELS if c == 0 else [])",
        ...
        "            spine.set_visible(False)",
        "",
        "    fig.suptitle(f'{PATH_NAMES[path]}  ·  raw counts (shade = share of that row\\'s own max)',",
        "                 fontweight='bold', y=1.00)",
        "    suffix = '' if THRESHOLD is None else f'_{int(THRESHOLD * 100)}pct'",
    ...
    md("md-3d", [
        "**Shading is per-row here** — each row is shaded against its own busiest cell, not",
        "against the other rows, since `answer.text` (55 items) and `section.description`",
        "(8 items) would otherwise be unreadable on one shared scale. Only the percentage",
        "matrix above compares cleanly across rows; this one is for the exact numbers.",
    ]),
    md("md-3b", [
