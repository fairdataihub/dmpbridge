...
cells = [
    ...
    md("md-3", [
        ...
        "",
        "Rows may not reach 100%: the remainder is items the model never produced at all.",
        "",
    ...
    code("matrix", [
        ...
        "    as j, out of ALL real items of class i (including ones never produced,",
        "    so the diagonal is exactly recall).\"\"\"",
        "    pct = np.zeros((len(LABELS), len(LABELS)))",
        "    for i, t in enumerate(LABELS):",
        "        row_total = sum(conf.get(t, {}).values())   # preds + never-produced",
        "        for j, p in enumerate(LABELS):",
        "            if row_total:",
        "                pct[i, j] = conf.get(t, {}).get(p, 0) / row_total",
        "    return pct",
        "",
        "",
        "cms = {(m, path): build_pct(CONF[m][path])",
        ...
        "fig.suptitle('Confusion matrices — % of each true class, all 10 documents',",
