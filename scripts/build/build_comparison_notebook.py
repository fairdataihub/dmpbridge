...
cells = [
    ...
    # ── 5 — the same two documents in Path B ────────────────────────────
    md("md-5", [
    ...
    md("md-5d", [
        "**The two models that wrote a real answer (`llama3.1:8b`, `llama3.3:70b`) are",
        "penalised; the one that left the field blank (`gemma4:e4b`) is rewarded.** That is",
        "backwards from what the metric is supposed to measure — this is a scoring artifact of",
        "the rule interacting with each model's own output, not a real difference in labeling",
        "ability. It is why the `(not labeled)` counts in Path B cannot be read the same way as",
        "Path A's: Path A's are caused by the extractor and apply equally to every model;",
        "Path B's depend on an incidental detail of how each model happened to fill one field.",
    ]),
]
