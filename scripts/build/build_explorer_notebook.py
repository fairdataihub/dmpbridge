...
cells = [
    ...
    md("md-settings", [
        ...
        "| `EXTRACTOR` | `'pdfplumber'` (the only one implemented) |",
    ...
    md("md-1b", [
        "**What just happened.** The whole PDF became one text string, with words visually",
        "emphasized relative to the document's own body-text font wrapped in `**bold**`",
        "markers and italics in `_markers_`. The model never sees the PDF itself — only this",
        "text, so anything lost here is lost for good.",
        "",
        "**What goes wrong here.** A PDF that renders text twice in offset layers (some",
        "PDFs' fake-bold trick) can double characters, since no char-level deduplication",
        "runs on this path. The model is also not perfectly reliable about stripping the",
        "`**`/`_` markers from its output before returning the classified text.",
        "",
